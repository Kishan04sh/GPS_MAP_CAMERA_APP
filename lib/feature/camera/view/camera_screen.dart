          ...
          Positioned(
            ...
            child: SafeArea(
              ...
              child: BottomInfoPanel(
                ...
                onVideo: cam.isReady && !_busy
                    ? () async {
                  _busy = true;

                  try {
                    debugPrint('[CameraScreen] switching to VIDEO');
                    // 1️⃣ Detach preview first
                    camNotifier.detachPreviewSafely.call();

                    // 2️⃣ Dispose photo camera
                    await camNotifier.disposeCamera();

                    // 3️⃣ HAL settle delay
                    await Future.delayed(const Duration(milliseconds: 150));

                    if (!mounted) return;

                    // 4️⃣ Navigate to video screen
                    ...
                    // 5️⃣ Re-init photo camera when back
                    if (mounted) {
                      debugPrint('[CameraScreen] re-init PHOTO camera');
                      await camNotifier.initCamera();
                    }
                  } catch (e, st) {
                    debugPrint('[CameraScreen][VIDEO SWITCH ERROR] $e');
                ...
                onCapture: cam.isReady && pos != null
                  ...
                  Navigator.push(
                    context,
                    MaterialPageRoute(
                      builder: (_) => FullImageView(file: stamped, position: pos,address:addr ?? '' ,),
                    ),
                  );
                }
                    : null,
